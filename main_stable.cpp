...
#define MAX_Y 256
#define MAX_Z 256
#define NUM 1                              //ограничение на количество мячей
#define MAX_R 10
...
//    cout << cubic_mesh << endl;
    return ((cubic_mesh->my_x0 < (x - R)) && (cubic_mesh->my_x1 > (x + R)) &&
            (cubic_mesh->my_y0 < (y - R)) && (cubic_mesh->my_y1 > (y + R)) &&
            (cubic_mesh->my_z0 < (z - R)) && (cubic_mesh->my_z1 > (z + R)) ||
            ((x - R) == 0 ) || ((x + R) == MAX_X) ||
            ((y - R) == 0 ) || ((y + R) == MAX_Y) ||
            ((z - R) == 0 ) || ((z + R) == MAX_Z));
}
...
void Ball::reallocate() {
    if (!am_I_inside()) {
        ...
        cout << "Остановились на " << cubic_mesh->my_x1 - cubic_mesh->my_x0 << endl;
        cubic_mesh->add(this);
    }
};
...
void Ball::about_me() {
    ...
    <<Ux <<"  Uy: "<< Uy  <<"  Uz: "<< Uz  <<") лежит в ячейке координатами: "
         << cubic_mesh->my_x0 << ':' << cubic_mesh->my_y0 << ':' << cubic_mesh->my_z0 << "  "
         << cubic_mesh->my_x1 << ':' << cubic_mesh->my_y1 << ':' << cubic_mesh->my_z1 << endl;
}
